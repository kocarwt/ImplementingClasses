...
class Point(object):
    ...
    def get_distance_from_start(self):
        x = self.x
        y = self.y
        dis = math.sqrt((self.x - self.orginalx)**2 + (self.y - self.orginaly)**2)
        return dis

    def get_distance_traveled(self):
        self.sum = abs(self.x - self.orginalx)+ abs(self.y - self.orginaly)
        return self.sum

    def closer_to(self, Point2, Point3):
        dis1 = math.sqrt((self.x - Point2.x)**2 + (self.y - Point2.y)**2)
        dis2 = math.sqrt((self.x - Point3.x)**2 + (self.y - Point3.y)**2)
        if dis1 < dis2:
            return Point2
        if dis2 < dis1:
            return Point3
